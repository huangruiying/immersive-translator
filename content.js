(function () {
  ...
  const TRANS_CLASS = 'it-translation';
  const BUBBLE_ID = 'it-bubble';

  function getSettings() {
    return new Promise((resolve) => {
      chrome.storage.sync.get(
        ['apiBase', 'apiKey', 'model', 'targetLang', 'prompt', 'mode'],
        (s) => resolve(s)
      );
    });
  }
  ...
  function sendTranslate(texts, settings, cb) {
    let attempt = 2;
    const go = () => {
      chrome.runtime.sendMessage({ type: 'TRANSLATE', texts, settings }, (resp) => {
  ...
  async function translatePage() {
    ...
    if (!settings.apiKey || !settings.apiBase) {
      isTranslating = false;
      if (confirm('尚未配置大模型端点或 API Key，是否打开设置？')) {
    ...
    for (let i = 0; i < texts.length; i += BATCH) {
      pendingBatches++;
      const groupNodes = nodes.slice(i, i + BATCH);
      const groupTexts = texts.slice(i, i + BATCH);
      sendTranslate(groupTexts, settings, (resp) => {
        if (resp && resp.translations) {
          groupNodes.forEach((node, k) => {
            const t = resp.translations[k];
            if (t) renderTranslation(node, t, settings.mode || 'bilingual');
          });
        } else if (resp && resp.error) {
          console.error('[immersive] translate error:', resp.error);
        }
        pendingBatches--;
        if (pendingBatches <= 0) isTranslating = false;
      });
    }
    if (pendingBatches === 0) isTranslating = false;
  }
  ...
  async function translateSelection(text) {
    ...
    const settings = await getSettings();
    if (!settings.apiKey || !settings.apiBase) {
      if (confirm('尚未配置大模型端点或 API Key，是否打开设置？')) chrome.runtime.sendMessage({ type: 'OPEN_OPTIONS' });
      return;
    }
    sendTranslate([text.trim()], settings, (resp) => {
  ...
  function showBubble(orig, trans) {
    ...
    if (!b) {
      ...
      b.id = BUBBLE_ID;
      b.innerHTML = '<div class="it-b-orig"></div><div class="it-b-trans"></div><div class="it-b-close">×</div>';
      document.body.appendChild(b);
      b.querySelector('.it-b-close').addEventListener('click', () => { b.style.display = 'none'; });
    }
    b.querySelector('.it-b-orig').textContent = orig;
    b.querySelector('.it-b-trans').textContent = trans;
  ...
  (function injectStyles() {
    ...
    s.textContent = `
      ...
      #${BUBBLE_ID} .it-b-orig { color:#9ca3af; font-size:12px; margin-bottom:4px; }
      #${BUBBLE_ID} .it-b-trans { color:#111; white-space: pre-wrap; }
      #${BUBBLE_ID} .it-b-close { position:absolute; top:2px; right:8px; cursor:pointer; color:#9ca3af; font-size:16px; }
    `;
